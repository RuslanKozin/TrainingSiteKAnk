...
include 'output.html.php';
    ...
    /^[\w\.\-]+@([\w\-]+\.)+[a-z]+$/i - расмотрим подробней:
        ...
        [\w\.\-]+ - фрагмент адреса с логином, состоит из одного или нескольких (знак "плюс") наборов
                    символов(слов), а также точек или дефисов.
        @ - символьное обозначение так называемой собачки.
        ([\w\-]+\.)+ - один или несколько (знак плюс) субдоменов (таких как sitepoint.,)каждый из которых состоит
                     из одного или нескольких наборов символов(слов) или дефисов с точкой в конце(\.).
        ...
        /i - закрывающий разделитель, обозначает конец регулярного выражения.
            Модификатор i после слеша указывает на то, что буквы в шаблоне [a-z] должны восприниматься без
                учета регистра.


        Функция preg_replace - принимает два параметра - регулярное выражение и строку, после чего пытается найти
                    в указанной строке заданный шаблон. Содержимое третьего параметра используется для замены
                    найденных совпадений.

            $newString = preg_replace(regExp, replaceWith, oldString);

                regExp - это регулярное выражение
                replaceWith - строка, которая заменяет собой все совпадения, обнаруженные в oldString.
            В качестве результата функция возвращает новую строку со всеми произведенными заменами, присваивая
                ее переменной $newString.
    */